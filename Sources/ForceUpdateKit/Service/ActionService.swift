...
public class ActionService: ActionServiceProtocol {
    ...
    public func action(request: ActionRequest) async throws -> ActionResponse? {
        do {
            guard let url = URL(string: "\(request.route)\(request.forecUpdateId)/") else {
            ...
            if (res as? HTTPURLResponse)?.statusCode == 204 {
                print("Force Update Action Response --> 204")
                return nil
            }
            if let response = try? JSONDecoder().decode(ActionResponse.self, from: data) {
                print("Force Update Action Response --> 200")
                print(response)
                return response
            } else {
                print("Force Update Action Response --> Decode Error")
                return nil
            }
        } catch {
            print("Failed to Force Update Action Request \(error)")
            return nil
        }
    }
}
